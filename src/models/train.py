...
def train(
    ...
    # 'PredConvAE.lr=1e-4.lambda=0.8.bz=32.seed=42_LD=16'
    params = {'mn': model_name, 'lr': lr, 'lambd': lambda_, 'ld': latent_dim, 'bs': batch_size, 'seed': seed,
              'cmt': experiment_name}
    full_exp_name = ''
    for key, value in params.items():
        full_exp_name += str(key) + '=' + str(value) + '.'

    # Initialize logging + create save folders
    os.makedirs(f"{save_path}/{full_exp_name}", exist_ok=True)
    os.makedirs(f"{logs_path}/{full_exp_name}", exist_ok=True)
    os.makedirs(f"{figure_path}/reconstructions/{full_exp_name}", exist_ok=True)
    writer = SummaryWriter(f"{logs_path}/{full_exp_name}")
    ...
    model, optimizer, recon_criterion, predictive_criterion = get_model(model_name=model_name, lr=lr,
    ...
    with trange(epochs) as t:
        for epoch in t:
            ...
            if epoch % visualize_every_epoch == 0:
                ...
                with torch.no_grad():
                    viz_batch = torch.stack([loaders['val'].dataset.__getitem__(i)['data'] for i in selected_idxs]).to(
                        device)
                ...
                plt.tight_layout()
                plt.savefig(f'{figure_path}/reconstructions/{full_exp_name}/epoch{epoch}.png')
                plt.close()
            ...
            for batch in iter(loaders['train']):
                # Get data
                x = batch['data'].to(device)  # .to(torch.float16)
                targets = batch['label'].to(device)  # .to(torch.float16)
            ...
            with torch.no_grad():
                for batch in iter(loaders['val']):
                    # Get data
                    x = batch['data'].to(device)  # .to(torch.float32)
                    targets = batch['label'].to(device)  # .to(torch.float32)
            ...
            t.set_description(
                f"EPOCH [{epoch + 1}/{epochs}] --> Train loss: {running_loss_train / Ntrain:.4f} | Val loss: {running_loss_val / Nval:.4f} | Val acc: {running_acc_val / Nval:.4f} | Progress: "
            )
            ...
            if running_loss_val < current_best_loss and epoch % checkpoint_every_epoch == 0:
                ...
                # Save checkpoint
                torch.save(checkpoint, f"{save_path}/{full_exp_name}/best.ckpt")
...
if __name__ == '__main__':
    ...
    train(
        ...
        lr=1e-4,
        lambda_=0.5,
        # weights predictive loss mostly --> higher lambda favors CrossEntropy loss, lower lambda favors reconstruction loss
        latent_dim=32,
        epochs=100,
